# personal-ip-video-agent

Fullstack EdgeSpark project.

## Customer delivery entrypoint

If this repository came from a customer delivery ZIP, read
`customer-deployment-and-agent-handoff.md` completely before changing code or
deploying. The package intentionally contains no production credentials,
deployment identity, user data, generated media, or payment merchant secrets.
Do not ask the customer to paste secrets into chat; use the EdgeSpark secure
secret flow or the target server's protected environment file.
